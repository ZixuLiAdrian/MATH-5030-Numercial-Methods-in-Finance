...
# Make the in-repo source tree importable when scripts are run directly.
_ROOT = Path(__file__).resolve().parents[1]
_SRC = _ROOT / "src"
if str(_SRC) not in sys.path:
    sys.path.insert(0, str(_SRC))

from asian_option_pricer import (  # noqa: E402  (path injection above is intentional)
    AsianOptionParams,
    antithetic_cv_price,
    rqmc_sobol_price,
)
...
def high_precision_reference(
    ...
    # Inverse-variance weighted average of the two independent estimators.
